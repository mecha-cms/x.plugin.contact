<form class="form-contact" id="<?php echo $anchor[1]; ?>" action="<?php echo $url->current; ?>/<?php echo Plugin::state('contact', 'path'); ?>" method="post">
  ...
  <p class="form-contact-select form-contact-select:title">
    ...
    <span><?php echo Form::select('*title', array_merge(["" => $language->contact_f_title], $b['topic']), null, ['class[]' => ['select', 'block'], 'id' => 'form-contact-select:title']); ?></span>
  </p>
  <?php else: ?>
  ...
  <p class="form-contact-button form-contact-button:state">
    ...
    <span><?php echo Form::submit('state', null, $language->contact_submit, ['class[]' => ['button', 'button-submit', 'set'], 'id' => 'form-contact-button:state']); ?></span>
  </p>
</form>